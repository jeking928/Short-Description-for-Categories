  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		...
		$categories_description_sub_array = $_POST['categories_description_sub'];
      ...
      // bof modified for Short Description for Categories
      'categories_description_sub' => ($categories_description_sub_array[$language_id] == '<p />' ? '' : zen_db_prepare_input($categories_description_sub_array[$language_id])),
      // eof modified for Short Description for Categories
...
<?php
  ...
  switch ($action) {
	...
	// bof modified for Short Description for Categories
	$category_sub_inputs_string .= '<br />' . zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;';
	// eof modified for Short Description for Categories
				
	// bof modified for Short Description for Categories
	$category_sub_inputs_string .= '<br />' . $output;
	// eof modified for Short Description for Categories
      $category_inputs_string .=  '<br />' .zen_draw_textarea_field('categories_description[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', htmlspecialchars(zen_get_category_description($cInfo->categories_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE));
	// bof modified for Short Description for Categories
	$category_sub_inputs_string .= '<br />' . zen_draw_textarea_field('categories_description_sub[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', zen_get_category_description_sub($cInfo->categories_id, $languages[$i]['id']));
	// eof modified for Short Description for Categories
    }
    $contents[] = array('text' => '<br />' . TEXT_CATEGORIES_DESCRIPTION . $category_inputs_string);
	// bof modified for Short Description for Categories
	$contents[] = array('text' => '<br />' . TEXT_CATEGORIES_DESCRIPTION_SUB . '<br />' . $category_sub_inputs_string);
	// eof modified for Short Description for Categories
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      $category_inputs_string .= '<br />' . zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' ;
      $category_sub_inputs_string .= '<br />' . zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' ;
      $category_inputs_string .= '<br />' . zen_draw_textarea_field('categories_description[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', zen_get_category_description($cInfo->categories_id, $languages[$i]['id']));
	// bof modified for Short Description for Categories
	$category_sub_inputs_string .= '<br />' . zen_draw_textarea_field('categories_description_sub[' . $languages[$i]['id'] . ']', 'soft', '100%', '20', zen_get_category_description_sub($cInfo->categories_id, $languages[$i]['id']));
	// eof modified for Short Description for Categories
    }
    $contents[] = array('text' => '<br />' . TEXT_CATEGORIES_DESCRIPTION . $category_inputs_string);
	// bof modified for Short Description for Categories
	$contents[] = array('text' => '<br />' . TEXT_CATEGORIES_DESCRIPTION_SUB . $category_sub_inputs_string);
	// eof modified for Short Description for Categories
